...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def on_examinar_pdf(self):
        ruta, _ = QFileDialog.getOpenFileName(
            self, "Seleccionar PDF", "", "Archivos PDF (*.pdf);;Todos los archivos (*)"
        )
        if not ruta:
            return
        self.ruta_pdf = ruta
        ...
        self.statusBar().showMessage(f"PDF cargado: {nombre}", 3000)
    ...
    def on_firmar_pdf(self):
        ...
        if not firmante or not motivo:
            QMessageBox.warning(self, "Aviso", "Introduce firmante y motivo.")
            return
        ruta_salida, _ = QFileDialog.getSaveFileName(
            self, "Guardar PDF firmado", "documento_firmado.pdf",
        ...
        QMessageBox.information(
            self, "Firma completada", f"PDF firmado correctamente.\nRuta:\n{ruta_salida}"
        )
